...
// no need to perform square root if the area is larger its root will also be larger.
// than the rest.
    }
//bubble sort
    for(int i=0;i<n;i++)    
    {
        for(int j=0;j<n-i-1;j++)
        {
            if(p[j]>p[j+1])     
            {
                ...
                p[j]=p[j+1];
                p[j+1]=temp;
                //swapping the actual elements in tr structure present in tr array according to the area value
                temp=tr[j].a;
                tr[j].a=tr[j+1].a;
                tr[j+1].a=temp;
                temp=tr[j].b;
